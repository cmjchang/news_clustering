...
def dynamic_clustering(inputs, delta, verbose=False, init_with_sc=False, fps=None, graph_weights=1e-5,
    ...
    # Inputs: inputs:                 input data array, with shape [n_frames, n_dimensions] delta:             the
    # hyper-parameter to control the complexity of cluster structure. delta=inf gives only one cluster and delta=0
    # gives n_frames cluster. init_with_sc:      If true, using the first 1-second frames to initialize the cluster
    # structure; else, the first feature vector is regarded as the initial cluster structure fps:               video
    # parameter, frames-per-second. Not used if init_with_sc=False graph_weights:     the edge weight of the
    # fully-connected graph for initialization. Not used if init_with_sc=False cluster_structure: If none,
    # the algorithm runs from scratch. Or the algorithm continues to update the ClusterStructure using the data
    # inputs, and other settings are overwritten by ClusterStructure. dist_type:         the metric to measure
    # distance between two samples. The default measure is the squared Euclidean distance
    ...
    if not init_with_sc:
        ...
        if verbose:
            print(
                '[RUNNING LOG] obtain the initial cluster structure via graph spectra, using the first 1-second '
                'features')
    ...
    ClusterStructureOut = {'centers': centers, 'radius': radius, 'NSamples': NSamples, 'M': M, 'idx': idx,
                           'delta': delta}
