...
from tools.preprocessing import sim_to_trad, cleaning, trad_to_sim
from tools.extract_tag import extract_tag
from tools.vectorization import vectorizer, dimension_reduction, doc_2_vec
...
google_df.loc[:, 'heading'] = trad_to_sim(google_df['heading'])
google_df.loc[:, 'content'] = trad_to_sim(google_df['content'])
...
google_tfidf = doc_2_vec(google_df)

#matrix = dimension_reduction(google_tfidf, method='nmf', n_components=25)

cluster_structure = dynamic_clustering(google_tfidf, delta=0.0002, verbose=False)
